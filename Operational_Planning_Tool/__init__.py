...
A specific *Configuration File* and its date must be chosen using *Set_ConfigFile. \n
...
*Check_ConfigFile* can be used to check if the values stated in the chosen *Configuration File* are plausible.
...
can be created by calling the *Create_ConfigFile* function. 
...
def Set_ConfigFile(Config_File_Name, Date):
    ...
    The Configuration file must be visible in sys.path.
