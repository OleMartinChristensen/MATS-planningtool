...
from pylab import norm, transpose, zeros, sqrt, floor, figure, plot, plot_date, datestr2num
import ephem, logging, csv

#from OPT_Config_File import Timeline_settings

import OPT_Config_File
from Operational_Planning_Tool import Library, MATS_coordinates
...
def Data_Plotter():
    ...
    date = ephem.Date(OPT_Config_File.Timeline_settings()['start_date'])
    ...
    LP_altitude = OPT_Config_File.Mode120_settings()['LP_pointing_altitude']/1000  #Altitude at which MATS center of FOV is looking [km]
    LP_altitude = 227
    ...
    for t in range(timesteps):
        ...
        if(t != 0):
            ...
            if( abs(lat_MATS[t])-abs(lat_MATS[t-1]) > 0 ): #Moving towards poles meaning LP is equatorwards compared to MATS
                ...
                R_LP = Library.lat_2_R(abs_lat_LP[t][0]) #Estimated WGS84 radius of LP from latitude of MATS
            else:
                abs_lat_LP[t] = abs(lat_MATS[t])+orbangle_between_LP_MATS/180*pi #absolute value of estimated latitude of LP in radians
                R_LP = Library.lat_2_R(abs_lat_LP[t][0]) #Estimated WGS84 radius of LP from latitude of MATS
            ...
            if( yaw_correction == True):
                ...
                yaw_offset_angle[t] = -30.8 * cos( arg_of_lat/180*pi - orbangle_between_LP_MATS/180*pi - 20/180*pi )
            ...
            rot_mat = Library.rot_arbit(pitch_MATS[t][0]/180*pi, normal_orbit[t,0:3])
            ...
            rot_mat = Library.rot_arbit( (-yaw_offset_angle[t][0])/180*pi, r_MATS_unit_vector[t,0:3])
    ...
    for t in range(timesteps):
        ...
        LP_ECEF_OHB[t,0], LP_ECEF_OHB[t,1], LP_ECEF_OHB[t,2] = MATS_coordinates.ecef2tanpoint(r_MATS_OHB_ECEF[t][0]*1000, r_MATS_OHB_ECEF[t][1]*1000, r_MATS_OHB_ECEF[t][2]*1000, 
    ...
    figure()
    plot_date(current_time_MPL[1:],yaw_offset_angle[1:], markersize = 1)
    plot_date(current_time_MPL[1:],Euler_angles_SLOF[1:,0], markersize = 1)
    
    
    figure()
    plot_date(current_time_MPL[1:],pitch_MATS[1:] , markersize = 1)
    plot_date(current_time_MPL[1:],Euler_angles_SLOF[1:,1], markersize = 1)
    
    figure()
    plot_date(current_time_MPL[1:],lat_MATS[1:]/pi*180, markersize = 1)
    plot_date(current_time_MPL[1:], lat_MATS_OHB[1:], markersize = 1)
    
    figure()
    plot_date(current_time_MPL[1:],long_MATS[1:]/pi*180, markersize = 1)
    plot_date(current_time_MPL[1:], long_MATS_OHB[1:], markersize = 1)
    
    figure()
    plot_date(current_time_MPL[1:],alt_MATS[1:] *1000, markersize = 1)
    plot_date(current_time_MPL[1:],alt_MATS_OHB[1:], markersize = 1)
    
    figure()
    plot_date(current_time_MPL[1:], lat_LP[1:], markersize = 1)
    plot_date(current_time_MPL[1:], lat_LP_OHB[1:], markersize = 1)
    
    figure()
    plot_date(current_time_MPL[1:],long_LP[1:], markersize = 1)
    plot_date(current_time_MPL[1:],long_LP_OHB[1:], markersize = 1)
    
    figure()
    plot_date(current_time_MPL[1:],alt_LP[1:], markersize = 1)
    plot_date(current_time_MPL[1:],alt_LP_OHB[1:], markersize = 1)
    
    
    
    figure()
    plot_date(current_time_MPL[1:],Ra[1:], markersize = 1)
    plot_date(current_time_MPL[1:],Ra_OHB[1:], markersize = 1)
    plot_date(current_time_MPL[1:],Ra_STK[1:], markersize = 1)
    
    figure()
    plot_date(current_time_MPL[1:],Dec[1:], markersize = 1)
    plot_date(current_time_MPL[1:],Dec_OHB[1:], markersize = 1)
