...
def date_calculator():
    ...
    time_skip_counter = 0
    timeskip = Settings['TimeSkip'] #Days to skip after each orbit
    ...
    while(current_time < timeline_start+ephem.second*duration):
        ...
        if(t != 0):
            ...
            for x in range(ROWS):
                ...
                stars_r_H_offset_plane[x] = stars_r[0][x] - (dot(stars_r[0][x],r_H_offset_normal[t,0:3]) * r_H_offset_normal[t]) 
...
def date_select(Occupied_Timeline, date_magnitude_array):
    ...
    while(restart == True):
        ...
        "If the all the magnitudes have been set arbitrary low or there are no simulated dates left (date=0) -> No Date available -> Exit"
