function animation_satview_los(filename, Vat_Pertur_t, limb_radiance_t,...
    x, y_t, z, t, zenithangle, h_field, yq, zq)

fig=figure('Visible','off','Position',[0 0 570 700]);   
colormap parula
...
%     set(gca,'color','none','FontSize',18)
    ...
    set(gca, 'YDir', 'normal')
    xlabel('Across track [km]')
%     ylabel('Tangent altitude [km]')
    colorbar('eastoutside');
    ylabel(colorbar,'Limb Radiance [cm^{-3} s^{-1}]','FontSize',18);
%     hold on
...
%     ylabel('Altitude [km]');
    ...
    colorbar('eastoutside');
    ylabel(colorbar,'VER [cm^{-3} s^{-1}]','FontSize',18);
