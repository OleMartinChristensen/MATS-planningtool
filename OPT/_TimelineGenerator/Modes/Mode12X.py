...
def UserProvidedDateScheduler(Occupied_Timeline, Settings):
    ...
    endDate = ephem.Date(StartDate+ephem.second* (Settings['freeze_start'] + Settings['freeze_duration'] + Timeline_settings['mode_separation']) )
    ...
    comment = str(Mode_name)+' scheduled using a user given date, the date got postponed '+str(iterations)+' times'
    ...
    if( Settings['automatic'] == False ):
        ...
        while(current_time < timeline_start+ephem.second*duration):
            ...
            for x in range(ROWS):
                ...
                stars_r_V_offset_plane[x] = stars_r[0][x] - (dot(stars_r[0][x],r_V_offset_normal[t]) * r_V_offset_normal[t])
