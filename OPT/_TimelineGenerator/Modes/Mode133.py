...
def Mode133(Occupied_Timeline):
    ...
    if( len(Settings['Exp_Times_UV']) <= len(Settings['Exp_Times_IR'])):
        duration = ( Settings['session_duration']+ Timeline_settings['CMD_separation'] * NumberOfCMDsPerAltitude +Timeline_settings['pointing_stabilization'] ) * len(Settings['Exp_Times_UV'])
    elif( len(Settings['Exp_Times_IR']) < len(Settings['Exp_Times_UV']) ):
        duration = ( Settings['session_duration']+ Timeline_settings['CMD_separation'] * NumberOfCMDsPerAltitude + Timeline_settings['pointing_stabilization'] ) * len(Settings['Exp_Times_IR'])
